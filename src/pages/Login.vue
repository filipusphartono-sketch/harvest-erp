...
const handleGoogleSignIn = async () => {
  ...
  try {
    ...
    await setDoc(doc(db, 'users', result.user.uid), {
      uid: result.user.uid,
      fullName: result.user.displayName || 'User Google',
      email: result.user.email,
      role: 'Karyawan',
      status: 'Aktif',
      // Merge true allows us to update existing without overwriting everything
    }, { merge: true })

    successMessage.value = 'Berhasil masuk dengan Google! Mengalihkan...'
    setTimeout(() => {
      router.push('/dashboard')
    }, 1000)
  } catch (err: any) {
    ...
    if (err.code === 'auth/operation-not-allowed') {
      error.value = 'Metode login Google belum diaktifkan di konfigurasi Firebase.'
    ...
    <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md z-10">
      <div class="bg-slate-900/60 backdrop-blur-xl py-8 px-4 border border-slate-800/80 shadow-2xl rounded-2xl sm:px-10">
        <form class="space-y-6" @submit.prevent="handleSubmit">
          ...
          <div v-if="!isForgotPassword" class="mt-6">
            <div class="relative flex items-center">
              <div class="flex-grow border-t border-slate-700"></div>
              <span class="flex-shrink-0 mx-4 text-sm text-slate-400">Atau lanjutkan dengan</span>
              <div class="flex-grow border-t border-slate-700"></div>
            </div>
            ...
            <div class="mt-6">
              <button
                ...
                :disabled="loading"
                class="w-full flex justify-center items-center py-2.5 px-4 border border-slate-700 rounded-lg shadow-sm bg-slate-800/50 text-sm font-medium text-slate-300 hover:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-950 focus:ring-purple-500 transition-all cursor-pointer disabled:opacity-50"
              >
